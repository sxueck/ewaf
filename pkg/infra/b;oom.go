...
// Filter is the struct of BloomFilter
...
// NewBloom New is function of creating a bloom filter
// k is number of hash function,
// m is the size of filter
// race is sync or not
func NewBloom(size uint64, k uint64, race bool) *Filter {
...
func baseHash(data []byte) []uint64 {
	...
	hashes := murmur3.New128()
	hashes.Write(data) // #nosec
	v1, v2 := hashes.Sum128()
	hashes.Write(a1) // #nosec
	v3, v4 := hashes.Sum128()
	return []uint64{
		v1, v2, v3, v4,
	}
...
// Reset the bits to zero used in filter
func (f *Filter) Reset() {
	if f.concurrent {
		f.lock.Lock()
		defer f.lock.Unlock()
